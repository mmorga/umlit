...
f = IO.foreach("sample.wsd") {|line|
  if  /title (.+)/ =~ line
    ...
    unless node_order.include? lnode
      node_order << lnode
    end
    unless node_order.include? rnode
      node_order << rnode
    end
    rowy += 40
  elsif /(alt|opt|loop) (.+)/ =~ line   
    boxes << {:type => $1.strip, :message => $2.strip, :start => rowy, :elses => [], :depth => depth}
    depth += 1
    rowy += 20
  elsif /else (.+)/ =~ line
    boxes.last[:elses] << {:row => rowy, :message => $1.strip}
    rowy += 20
  elsif line.strip == 'end'
    last = boxes.size - 1
    while (boxes[last].has_key?(:end))
      last -= 1
    end
    boxes[last][:end] = rowy
    rowy += 15
...
node_order.each_with_index {|node, idx|
  x = idx * 100;
